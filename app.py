...
# Traço de linha por fonte — cor E estilo diferentes, para ajudar a distinguir mesmo quando as linhas se cruzam
SOURCE_STYLE = {
    "Referência": dict(color=REF_COLOR, width=3, dash="solid"),
    "Sensor 3min": dict(color=S1_COLOR, width=2, dash="dash"),
    "Sensor 5min": dict(color=S2_COLOR, width=2, dash="dot"),
}


def source_selector(key_prefix, default=None):
    """Checkboxes para ligar/desligar cada linha do gráfico — mais fácil que usar a legenda."""
    st.caption("👁️ Quais fontes mostrar no gráfico? (dica: clicar numa linha da legenda também esconde/mostra)")
    opcoes = ["Referência", "Sensor 3min", "Sensor 5min"]
    default = default or opcoes
    cols = st.columns(len(opcoes))
    selecionadas = []
    for col, nome in zip(cols, opcoes):
        with col:
            if st.checkbox(nome, value=(nome in default), key=f"{key_prefix}_show_{nome}"):
                selecionadas.append(nome)
    return selecionadas


def render_comparison_chart(d, ref_col, s1_col, s2_col, variavel, unidade, key_prefix, height=420):
    """Gráfico de série temporal com seletor de fontes e traços distintos por linha."""
    fontes_visiveis = source_selector(key_prefix)

    col_map = {"Referência": ref_col, "Sensor 3min": s1_col, "Sensor 5min": s2_col}
    fig = go.Figure()
    faltando = []
    for nome in ["Referência", "Sensor 3min", "Sensor 5min"]:
        if nome not in fontes_visiveis:
            continue
        col = col_map[nome]
        if col in d.columns and d[col].notna().any():
            fig.add_trace(go.Scatter(x=d["data_hora_cheia"], y=d[col], name=nome,
                                      line=SOURCE_STYLE[nome]))
        else:
            faltando.append(nome)
    if faltando:
        st.caption("⚠️ Sem dados para: " + ", ".join(faltando) + f" (`{variavel}`).")
    fig.update_layout(height=height, yaxis_title=f"{variavel} ({unidade})", xaxis_title="Data/Hora",
...
# TABS
# ============================================================
tab_overview, tab_ref, tab_s1, tab_s2, tab_compare, tab_multi, tab_stats = st.tabs(
    ["📊 Visão Geral", "🏛️ Referência (Prefeitura)", "🟠 Sensor 3min", "🔵 Sensor 5min",
     "📈 Comparação & Regressão", "📐 Regressão Múltipla", "📋 Estatísticas Resumo"]
)
...
# ------------------------------------------------------------
# TAB 6 — REGRESSÃO LINEAR MÚLTIPLA
# ------------------------------------------------------------
with tab_multi:
    st.subheader("📐 Regressão Linear Múltipla")
    st.caption(
        "Construa um modelo que prevê a referência a partir de mais de uma variável do sensor "
        "de baixo custo ao mesmo tempo — por exemplo, PM2.5 bruto + temperatura + umidade. "
        "Isso costuma melhorar a calibração, porque sensores ópticos de baixo custo sofrem "
        "influência da umidade e da temperatura do ar."
    )
    ...
    if df_merged is None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            sensor_choice = st.radio("Sensor", ["Sensor 3min", "Sensor 5min"], horizontal=True, key="mr_sensor")
        with c2:
            alvo_choice = st.radio("O que queremos prever (variável alvo)?",
                                    ["PM2.5 (Referência)", "PM10 (Referência)"],
                                    horizontal=True, key="mr_alvo")

        n = "1" if sensor_choice == "Sensor 3min" else "2"
        target_col = "ref_pm25" if "PM2.5" in alvo_choice else "ref_pm10"
        pol_default = "pm25" if "PM2.5" in alvo_choice else "pm10"

        predictor_map_all = {
            "PM2.5 bruto": f"s{n}_pm25_raw",
            "PM2.5 corrigido": f"s{n}_pm25_corr",
            "PM10 bruto": f"s{n}_pm10_raw",
            "PM10 corrigido": f"s{n}_pm10_corr",
            "Temperatura": f"s{n}_temp",
            "Umidade": f"s{n}_umid",
        }
        ...
        if not predictor_map:
            st.warning("Não há variáveis preditoras disponíveis para esse sensor nos dados enviados.")
        else:
            default_label = f"PM{'2.5' if pol_default == 'pm25' else '10'} bruto"
            default_preditoras = [l for l in [default_label, "Temperatura", "Umidade"] if l in predictor_map]
            preditoras_sel = st.multiselect(
                "🧮 Variáveis preditoras (o que vai entrar na equação)",
                list(predictor_map.keys()), default=default_preditoras, key="mr_preditoras",
                help="Escolha 2 ou mais variáveis. Recomenda-se começar com o valor bruto do poluente-alvo "
                     "+ temperatura + umidade.",
            )

            if len(preditoras_sel) < 1:
                st.info("Selecione ao menos uma variável preditora.")
            else:
                cols_needed = [target_col] + [predictor_map[p] for p in preditoras_sel]
                dd = d.dropna(subset=cols_needed)

                if len(dd) < len(preditoras_sel) + 5:
                    st.warning(f"Poucos dados válidos ({len(dd)} linhas) para ajustar um modelo confiável com "
                               f"{len(preditoras_sel)} variável(is). Tente reduzir preditoras ou ampliar o período.")
                else:
                    X_cols = [predictor_map[p] for p in preditoras_sel]
                    X = sm.add_constant(dd[X_cols])
                    y = dd[target_col]
                    model = sm.OLS(y, X).fit()
                    nome_alvo = "PM2.5 Referência" if target_col == "ref_pm25" else "PM10 Referência"

                    st.markdown("#### Equação ajustada")
                    termos = " + ".join([f"{model.params[c]:.3f}·{label}" for c, label in zip(X_cols, preditoras_sel)])
                    st.markdown(f"`{nome_alvo} ≈ {model.params['const']:.3f} + {termos}`")

                    st.markdown("#### Coeficientes")
                    coef_table = pd.DataFrame({
                        "Variável": ["Intercepto"] + preditoras_sel,
                        "Coeficiente": model.params.values,
                        "Erro padrão": model.bse.values,
                        "t": model.tvalues.values,
                        "p-valor": model.pvalues.values,
                        "IC 95% (inf)": model.conf_int()[0].values,
                        "IC 95% (sup)": model.conf_int()[1].values,
                    })
                    coef_table["Significância (α=0.05)"] = coef_table["p-valor"].apply(
                        lambda p: "✅ significativo" if p < 0.05 else "— não significativo")
                    st.dataframe(
                        coef_table.style.format({
                            "Coeficiente": "{:.4f}", "Erro padrão": "{:.4f}", "t": "{:.3f}",
                            "p-valor": "{:.4f}", "IC 95% (inf)": "{:.4f}", "IC 95% (sup)": "{:.4f}",
                        }),
                    ...
                    m2.metric("R² ajustado", f"{model.rsquared_adj:.4f}")
                    m3.metric("RMSE", f"{rmse:.3f}")
                    m4.metric("n (observações)", f"{len(dd)}")

                    col_pm_bruto = predictor_map.get(default_label)
                    if col_pm_bruto and col_pm_bruto in dd.columns:
                        X_simple = sm.add_constant(dd[[col_pm_bruto]])
                        model_simple = sm.OLS(y, X_simple).fit()
                        delta = model.rsquared - model_simple.rsquared
                        st.caption(
                            f"📈 Comparado a usar só o valor bruto como preditor (R² = {model_simple.rsquared:.4f}), "
                            f"o modelo múltiplo {'melhora' if delta > 0 else 'não melhora'} o ajuste em "
                            f"{delta:+.4f} de R²."
                        )

                    colA, colB = st.columns(2)
                    with colA:
                        st.markdown("**Previsto vs. Real**")
                        fig_pred = px.scatter(x=y, y=y_pred, opacity=0.6,
                                               labels={"x": f"{nome_alvo} — real", "y": f"{nome_alvo} — previsto"})
                        line = np.linspace(min(y.min(), y_pred.min()), max(y.max(), y_pred.max()), 50)
                        fig_pred.add_trace(go.Scatter(x=line, y=line, mode="lines", name="Identidade (y=x)",
                                                       line=dict(color="gray", dash="dash")))
                        fig_pred.update_layout(height=360, margin=dict(t=20), showlegend=False)
                        st.plotly_chart(fig_pred, use_container_width=True)
                    with colB:
                        st.markdown("**Resíduos vs. Previsto**")
                        fig_res = px.scatter(x=y_pred, y=resid, opacity=0.6,
                                              labels={"x": f"{nome_alvo} — previsto", "y": "Resíduo (real − previsto)"})
                        fig_res.add_hline(y=0, line_dash="dash", line_color="gray")
                        fig_res.update_layout(height=360, margin=dict(t=20))
                        st.plotly_chart(fig_res, use_container_width=True)
                    st.caption(
                        "No gráfico de resíduos, o ideal é uma nuvem de pontos espalhada aleatoriamente em torno "
                        "de zero, sem formar curvas ou funis — isso indicaria que o modelo linear não captura "
                        "bem a relação."
                    )

                    if len(preditoras_sel) >= 2:
                        corr_pred = dd[X_cols].corr()
                        pares_altos = []
                        for i in range(len(X_cols)):
                            for j in range(i + 1, len(X_cols)):
                                r = corr_pred.iloc[i, j]
                                if abs(r) > 0.8:
                                    pares_altos.append((preditoras_sel[i], preditoras_sel[j], r))
                        if pares_altos:
                            st.warning(
                                "⚠️ **Possível multicolinearidade** — estas variáveis preditoras estão fortemente "
                                "correlacionadas entre si, o que pode tornar os coeficientes individuais instáveis:\n\n"
                                + "\n".join([f"- {a} × {b}: r = {r:.2f}" for a, b, r in pares_altos])
                            )

                    with st.expander("📖 O que é regressão linear múltipla?"):
                        st.markdown(
                            "- A regressão múltipla estima a referência combinando **duas ou mais variáveis** ao "
                            "mesmo tempo (ex.: PM bruto + temperatura + umidade), em vez de usar só uma.\n"
                            "- **R² ajustado**: parecido com o R², mas penaliza adicionar preditoras que não ajudam "
                            "de verdade — compare-o com o R² comum para ver se vale a pena manter todas as variáveis.\n"
                            "- **p-valor de cada coeficiente**: indica se aquela variável, isoladamente, contribui "
                            "de forma estatisticamente significativa (convenção: p < 0.05) para prever a referência, "
                            "mantendo as demais fixas.\n"
                            "- **Multicolinearidade**: quando dois preditores carregam quase a mesma informação "
                            "(ex.: PM2.5 bruto e PM2.5 corrigido), o modelo tem dificuldade em separar o efeito de "
                            "cada um — os coeficientes individuais ficam instáveis mesmo que a previsão geral "
                            "continue boa.\n"
                            "- Esse modelo é **estatístico** (correlação), não prova uma relação física de causa "
                            "e efeito — mas é exatamente esse tipo de ajuste que costuma ser usado para calibrar "
                            "sensores de baixo custo contra uma referência."
                        )

                    out_df = dd[["data_hora_cheia", target_col] + X_cols].copy()
                    out_df["previsto"] = y_pred
                    out_df["residuo"] = resid
                    st.download_button(
                        "⬇️ Baixar dados + previsão do modelo (CSV)",
                        out_df.to_csv(index=False).encode("utf-8"),
                        f"regressao_multipla_{sensor_choice}_{target_col}.csv", "text/csv", key="dl_mr",
                    )

# ------------------------------------------------------------
# TAB 7 — ESTATÍSTICAS RESUMO
# ------------------------------------------------------------
with tab_stats:
